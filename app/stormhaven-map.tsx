...
function createCity(selectDistrict:(name:string)=>void){
  ...
  const cobble=new THREE.MeshStandardMaterial({color:0x202b2d,roughness:.72,metalness:.15}),paleStone=new THREE.MeshStandardMaterial({color:0x6f716c,roughness:.9}),arcLamp=new THREE.MeshBasicMaterial({color:0x76e8ff}),gasLamp=new THREE.MeshBasicMaterial({color:0xe2a258}),redCanvas=new THREE.MeshStandardMaterial({color:0x784638,roughness:.82}),tealCanvas=new THREE.MeshStandardMaterial({color:0x35666a,roughness:.82});
  const bandLower=new THREE.MeshBasicMaterial({color:0xa86d4b,transparent:true,opacity:.12,depthWrite:false,side:THREE.DoubleSide}),bandMid=new THREE.MeshBasicMaterial({color:0x5a9a9b,transparent:true,opacity:.1,depthWrite:false,side:THREE.DoubleSide}),bandUpper=new THREE.MeshBasicMaterial({color:0xb79a68,transparent:true,opacity:.105,depthWrite:false,side:THREE.DoubleSide});
  ...
  addUrbanGrid(wards,{name:"Lower City fabric",short:"Lower City",x:61,y:11,z:12,color:"#a86d4b",kind:"",population:"",description:""},17,5,28,7,121,architecture,["tenement","canal-house","warehouse"],.46,{openCore:1.4});
  addUrbanGrid(wards,{name:"Mid-City fabric",short:"Mid-City",x:61,y:28,z:28,color:"#5a9a9b",kind:"",population:"",description:""},15,5,31,8,132,architecture,["tenement","townhouse","canal-house","glassworks"],.58,{openCore:1.7});
  addUrbanGrid(wards,{name:"Upper City fabric",short:"Upper City",x:48,y:43,z:40,color:"#b79a68",kind:"",population:"",description:""},11,5,22,7,143,architecture,["townhouse","shrine","greenhouse"],.58,{openCore:2.1});
  ...
  const grove=new THREE.Group(),groveRandom=seeded(7521);for(let i=0;i<62;i++){const x=44+(groveRandom()-.5)*11,y=49+(groveRandom()-.5)*9,p=cityPos(x,y,Math.min(45,terrainHeight(x,y))),trunk=new THREE.Mesh(new THREE.CylinderGeometry(.06,.1,.7+groveRandom()*.5,5),wetWood),crown=new THREE.Mesh(new THREE.IcosahedronGeometry(.38+groveRandom()*.38,1),garden);trunk.position.set(p.x,p.y+.45,p.z);crown.position.set(p.x,p.y+1+groveRandom()*.7,p.z);grove.add(trunk,crown)}city.add(grove);
...
export function StormhavenMap(){
  ...
  useEffect(()=>{
    ...
    const mobileGrade=window.matchMedia("(max-width: 760px)").matches;
    const scene=new THREE.Scene();scene.background=new THREE.Color(mobileGrade?0x102128:0x05090c);scene.fog=new THREE.FogExp2(mobileGrade?0x142b31:0x071116,mobileGrade ? .0102 : .0125);
    const camera=new THREE.PerspectiveCamera(38,host.clientWidth/host.clientHeight,.035,240);camera.position.set(-47,43,62);
    const renderer=new THREE.WebGLRenderer({antialias:true,alpha:false,powerPreference:"high-performance"});renderer.setPixelRatio(Math.min(window.devicePixelRatio,mobileGrade?1.25:1.75));renderer.setSize(host.clientWidth,host.clientHeight);renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=mobileGrade?1.04:.86;renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFShadowMap;host.appendChild(renderer.domElement);
    ...
    scene.add(new THREE.HemisphereLight(mobileGrade?0x86afba:0x5d8190,mobileGrade?0x29241f:0x17100e,mobileGrade?1.72:1.2));const stormLight=new THREE.DirectionalLight(0xb7dae1,mobileGrade?3.85:3.3);stormLight.position.set(-28,52,22);stormLight.castShadow=true;stormLight.shadow.mapSize.set(mobileGrade?1024:2048,mobileGrade?1024:2048);stormLight.shadow.camera.left=-55;stormLight.shadow.camera.right=55;stormLight.shadow.camera.top=42;stormLight.shadow.camera.bottom=-42;scene.add(stormLight);const copperGlow=new THREE.PointLight(0xd47842,mobileGrade?21:18,34,2);copperGlow.position.set(-19,5,13);scene.add(copperGlow);
    if(mobileGrade){
    ...
    const clouds=new THREE.Group(),cloudMaterial=new THREE.MeshStandardMaterial({color:0x0c1419,transparent:true,opacity:mobileGrade ? .58 : .72,roughness:1,depthWrite:false}),cloudRandom=seeded(903),cloudCount=mobileGrade?18:34;for(let i=0;i<cloudCount;i++){const cloud=new THREE.Mesh(new THREE.IcosahedronGeometry(5+cloudRandom()*9,mobileGrade?1:2),cloudMaterial);cloud.scale.y=.16+cloudRandom()*.11;cloud.position.set((cloudRandom()-.5)*115,20+cloudRandom()*8,(cloudRandom()-.5)*75);clouds.add(cloud)}scene.add(clouds);
    const rainCount=mobileGrade?900:2400,rainPositions=new Float32Array(rainCount*3),rainRandom=seeded(8844);for(let i=0;i<rainCount;i++){rainPositions[i*3]=(rainRandom()-.5)*105;rainPositions[i*3+1]=rainRandom()*34;rainPositions[i*3+2]=(rainRandom()-.5)*74}const rainGeometry=new THREE.BufferGeometry();rainGeometry.setAttribute("position",new THREE.BufferAttribute(rainPositions,3));const rain=new THREE.Points(rainGeometry,new THREE.PointsMaterial({color:0xa3c8cc,size:mobileGrade ? .019 : .025,transparent:true,opacity:mobileGrade ? .25 : .42}));scene.add(rain);
